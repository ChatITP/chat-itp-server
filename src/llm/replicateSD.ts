 ...
 */
async function generateImage(prompt: string, width: number = 1024, height: number = 1024): Promise<{ success: boolean, content: string }> {
    console.log(prompt);
  try {
    ...
    return { success: false, content: "Failed to generate image. Please try again or use a different prompt." };
  }
}
...
async function isImageGenerationRequest(userPrompt: string): Promise<boolean> {
    ...
    const result = Array.isArray(output) ? output.join("").trim().toUpperCase() : String(output).trim().toUpperCase();
